...
export function calculateSubsidy(power: number): number {
  // Récupérer les valeurs des primes depuis le localStorage
  const subsidyUnder3kw = parseFloat(localStorage.getItem('default_subsidy_under_3kw') || '80');
  const subsidyOver3kw = parseFloat(localStorage.getItem('default_subsidy_over_3kw') || '80');
  const subsidy9to36kw = parseFloat(localStorage.getItem('default_subsidy_9to36kw') || '190');
  const subsidy36to100kw = parseFloat(localStorage.getItem('default_subsidy_36to100kw') || '100');

  // Calcul de la prime en fonction de la puissance
  let primeParKwc = 0;
  if (power <= 3) {
    primeParKwc = subsidyUnder3kw;
  } else if (power <= 9) {
    primeParKwc = subsidyOver3kw;
  } else if (power <= 36) {
    primeParKwc = subsidy9to36kw;
  } else if (power <= 100) {
    primeParKwc = subsidy36to100kw;
  }

  // Calcul de la prime totale (puissance en kWc * prime par kWc)
  return Math.round(power * primeParKwc);
}
...
export function calculateFinalPrice(
  ...
  // La remise est maintenant en euros, pas en pourcentage
  return basePrice - primeAutoconsommation - remiseCommerciale;
}