...
export async function getSuggestions(query: string): Promise<AddressFeature[]> {
  ...
  try {
    ...
    if (circuitBreaker.isOpen()) {
      console.log('Circuit breaker is open - using fallback data immediately');
    ...
    const timeoutId = setTimeout(() => {
      controller.abort();
      console.log('Aborting address API request due to timeout (20s)');
    ...
    try {
      ...
      }, 3); // Reduced retries from 5 to 3 to fail faster when service is down
