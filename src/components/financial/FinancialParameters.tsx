...
export default function FinancialParameters({ 
  ...
  useEffect(() => {
    ...
    // Si MyBattery est sélectionné, le tarif est basé sur le prix du kWh moins les taxes
    if (parameters.batterySelection?.type === 'mybattery') {
      tariff = Math.max(0, parameters.prixKwh - 0.0996);
      onParameterChange({ 
        primeAutoconsommation: 0, // Pas de prime avec MyBattery
        connectionType: 'surplus', // Force le type en surplus
        autoconsommation: settings.defaultAutoconsumption // Reset à la valeur par défaut
      });
    } else if (parameters.batterySelection?.type === 'virtual') {
      // Force auto-consommation totale pour Smart Battery
      onParameterChange({ 
    ...
    // Mise à jour de l'autoconsommation selon le type de raccordement
    ...
    // Ne pas appliquer la prime si MyBattery est sélectionné
    if (!parameters.batterySelection?.type === 'mybattery') {
  ...
  const handleConnectionTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    // Si MyBattery ou Smart Battery est sélectionné, empêcher le changement de type de raccordement
  ...
  const handleBatteryChange = (batterySelection: any) => {
    // Calcul du nouveau taux d'autoconsommation
    let newAutoconsommation = settings?.defaultAutoconsumption ?? 75; // Taux de base avec fallback

    if (batterySelection.type === 'physical' && batterySelection.model) {
      // Ajouter l'augmentation d'autoconsommation de la batterie
    ...
    onParameterChange({ 
      ...
      // Si MyBattery est sélectionné, forcer le type de raccordement en surplus et retirer la prime
