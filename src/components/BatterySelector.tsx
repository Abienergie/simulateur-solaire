...
import { Battery, BatteryCharging, CloudLightning, Car, CloudSun, Info } from 'lucide-react';
...
export default function BatterySelector({ 
  ...
  // Filter physical batteries based on subscription duration
  ...
  // Reset storage options when switching to total sale
  useEffect(() => {
  ...
  const handleTypeChange = (type: 'physical' | 'virtual' | 'mybattery' | null) => {
    const shouldResetAutoconsumption = value.type === 'virtual' && type === 'mybattery';
    
    onChange({ 
      type,
      includeSmartCharger: type === 'virtual' ? includeSmartCharger : false,
      resetAutoconsumption: shouldResetAutoconsumption ? initialAutoconsumption : undefined
    });
  };
  ...
  const TariffTooltip = () => (
    <div className="relative inline-block">
      <Info
        className="h-4 w-4 text-blue-400 cursor-help inline-block ml-1"
        onMouseEnter={() => setShowTariffTooltip(true)}
        onMouseLeave={() => setShowTariffTooltip(false)}
      />
      {showTariffTooltip && (
        <div className="absolute z-10 w-72 p-3 text-sm bg-gray-900 text-white rounded-lg -right-2 bottom-6">
  ...
  // Disable the entire component if connection type is total_sale
  if (connectionType === 'total_sale') {
    return (
  ...
  // Show physical battery options based on formula and settings
              ...
              >
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className="font-medium">
                    {batteryFormula === 'comptant' ? 'Batterie physique à l\'achat' : 'Batterie physique'}
                  </span>
                </div>
              </button>
            )}
          ...
          {value.type === 'virtual' && (
            <div className="space-y-4">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <h4 className="font-medium text-blue-900 mb-2">Smart Battery de MyLight</h4>
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <h4 className="font-medium text-green-900">MySmartCharger – La borne de recharge intelligente</h4>
                    </div>
                    <div className="flex items-center gap-2">
                      <input
                        type="checkbox"
            ...
            <div className="space-y-4">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-sm text-blue-800">
                    Le surplus récupéré = prix du kWh - {formatCurrency(0.0995)} de taxes
                  </p>
                </div>
              </div>
            </div>
