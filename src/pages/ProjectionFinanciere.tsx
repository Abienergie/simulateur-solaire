...
// Define power options based on meter type
const generatePowerOptions = (isMonophase: boolean) => {
  const maxPower = isMonophase ? 6.0 : 9.0;
  const options = [];
  for (let power = 3.0; power <= maxPower; power += 0.5) {
    options.push({
      value: power,
...
export default function ProjectionFinanciere() {
  ...
  // Generate power options based on meter type
  const powerOptions = generatePowerOptions(solarResults?.typeCompteur === 'monophase');
  ...
  // Check if calculation should be disabled due to Smart Battery without capacity
