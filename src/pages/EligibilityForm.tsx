...
export default function EligibilityForm() {
  ...
  const handleIcollRedirect = async () => {
    try {
      ...
      const packageId = getPackageId(powerInKw, duration);
      
      const clientData = {
        civilite: clientInfo.civilite,
        nom: clientInfo.nom,
        prenom: clientInfo.prenom,
        adresse: address.rue || clientInfo.adresse || '',
        codePostal: address.codePostal || clientInfo.codePostal || '',
        ville: address.ville || clientInfo.ville || '',
        ...
        revenuFiscal: localStorage.getItem('revenuFiscal') || undefined
      };

      setProcessingStep('Création du dossier...');
      const result = await icollApi.registerClientAndCreateQuote(clientData);
      
      setQuoteData(result);
      
      if (result.pdfUrl) {
        localStorage.setItem('quote_pdf_url', result.pdfUrl);
      }

      setProcessingStep('Redirection vers iColl...');
      window.location.href = icollApi.getIcollLoginUrl(result.clientId, commercialId, result.quoteId);
  ...
  const handleDownloadQuote = () => {
    const pdfUrl = quoteData?.pdfUrl || localStorage.getItem('quote_pdf_url');
    if (pdfUrl) {
            ...
            <div className="flex">
              ...
              <div className="w-1/2 p-6">
                ...
                <div className="space-y-3">
                  <button
                    onClick={handleIcollRedirect}
                    disabled={isProcessing}
                    className="w-full px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-wait"
                  >
                    {isProcessing ? (
                      <>
                        <Loader2 className="h-5 w-5 animate-spin" />
                        {processingStep || 'Préparation en cours...'}
                      </>
                    ) : (
                      <>
