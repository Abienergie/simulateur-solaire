...
export interface PromoCode {
  ...
  subscription_effect?: 'free_months' | 'free_deposit' | 'free_battery_setup';
  free_months?: number;
}
...
const SUBSCRIPTION_PROMO_CODES: PromoCode[] = [
  {
    id: 'cautionfree',
    code: 'CAUTIONFREE',
    discount: 0,
...
const LOCAL_PROMO_CODES: PromoCode[] = [
  {
    id: 'soleil2025',
    code: 'SOLEIL2025',
    discount: 500,
...
export function usePromoCode(financingMode: 'cash' | 'subscription' = 'cash') {
    ...
    try {
      ...
      if (subscriptionCode) {
        ...
        if (financingMode === 'subscription') {
          ...
          // In cash mode, subscription codes don't work (except BATTERYFREE)
  ...
  const updateValidPromoCodes = (newCode: PromoCode) => {
    setValidPromoCodes(prev => {
      ...
      const totalDiscount = updated.reduce((sum, code) => sum + (code.discount || 0), 0);
  ...
  const applyPromoCode = (code: string): ApplyPromoCodeResult => {
    ...
    if (subscriptionCode && subscriptionCode.code !== 'BATTERYFREE') {
